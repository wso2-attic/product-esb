...
public class ETagSupportTestCase extends ESBIntegrationTest {
    ...
    private final String epr = "http://localhost:8280/services/ETagTestProxy";
    ...
    public void init() throws Exception {
        ...
        loadESBConfigurationFromClasspath(File.separator + "artifacts" + File.separator + "ESB" +
                                          File.separator + "passthru" + File.separator +
                                          "transport" + File.separator + "eTagSupportTestProxy.xml");


    }

    @Test(groups = "wso2.esb", description = "ETag enable test for a pass-through proxy", enabled = false)
    public void testEnableETag() throws Exception {
        ...
        Assert.assertTrue(eTagHeader != null, "ETag header is not present in the response");

        int statusCode = response.getStatusLine().getStatusCode();
        Assert.assertTrue(statusCode == 200, "Incorrect status code. Expected: 200 Found: " + statusCode);
        Assert.assertTrue(httpClient.getResponsePayload(response) != null, "Payload should not empty");
    }
    ...
    public void testIfNoneMatchHeaderCacheHit() throws Exception {
        //headers.put("If-None-Match", "\"-16-97-102-122-100-29-106112-103-39117-127-648049-104\"");
        headers.put("If-None-Match", "\"" + eTagHeader.getValue() + "\"");

        HttpResponse response = httpClient.doPost(epr, headers, payload, "text/xml");

        int statusCode = response.getStatusLine().getStatusCode();
        Assert.assertTrue(statusCode == 304, "Incorrect status code. Expected: 304 Found: " + statusCode);
        Assert.assertTrue(httpClient.getResponsePayload(response) == null, "Payload should be empty");
