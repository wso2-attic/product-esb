 ...
 *
 */
public class ForwardingMessageProcessorWithJMSQueueTestCase extends
		ESBIntegrationTest {
	...
	public void testMessageForwardingProcessorWithJMSQueue() throws Exception {
		...
		for (LogEvent logEvent : logs) {
			...
			if (message.contains(getExpectedSOAPResponse())
					&& message.contains(expectedSymbol)
	...
	public void testMessageProcessorDeactivationOnMaxDelivaryAttempts()
		...
		final String expectedDeactivationMsg = "Successfully deactivated the message processor [Processor2]";
		final String expectedErrorMsg = "BlockingMessageSender of message processor [Processor2] failed to send message to the endpoint";
	...
	private String getExpectedSOAPResponse() {
		final String expectedResponse = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\">"
	...
	private OMElement getStockQuoteRequest(String symbol) {
		...
		OMNamespace omNs = fac.createOMNamespace("http://services.samples",
				"ns");
