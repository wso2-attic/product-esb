 ...
 */
public class ESBPOXSecurityByUnauthorizedUserTestCase extends ESBIntegrationTest {
    ...
    public void testAddNewStudent() throws IOException, EndpointAdminEndpointAdminException,
        ...
        try {
            ...
            // invalid users cannot update the resource
            status = ignored.getMessage().contains("401");  // HTTP error code 401 sent from server
        }
        assertTrue(status, "Invalid user was not able to add the resource");
    }
        ...
        try {
            HttpsResponse response =
                    HttpsURLConnectionClient.putWithBasicAuth(securedRestURL, updateStudentData,
                            "application/xml", "invalidUser","invalidPassword");
        } catch (IOException ignored) {
            // invalid users cannot update the resource
            status = ignored.getMessage().contains("401");  // HTTP error code 401 sent from server
        }
        assertTrue(status, "Invalid user was not able to update the resource");
    }
    ...
    public void testDeleteStudent() throws IOException, EndpointAdminEndpointAdminException,
        ...
        try {
            ...
            // invalid users cannot get the resource
            status = ignored.getMessage().contains("401");
        }
    ...
    public void testGetResourceAfterDelete()
        ...
        try {
            ...
            // invalid users cannot get the resource
            getStatus = ignored.getMessage().contains("401");
        }
