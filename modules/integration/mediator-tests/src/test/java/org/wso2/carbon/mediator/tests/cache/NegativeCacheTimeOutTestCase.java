...
/**
 * This class will test cache mediator which has negative value to cache timeout property
 * in it's configuration
 */
public class NegativeCacheTimeOutTestCase extends ESBIntegrationTestCase {
    private StockQuoteClient axis2Client;

    public void init() throws Exception {
        axis2Client = new StockQuoteClient();
        launchBackendAxis2Service(SampleAxis2Server.SIMPLE_STOCK_QUOTE_SERVICE);
        String filePath = "/mediators/cache/negative_timeout.xml";
    ...
    public void testMessageSizeProperty() throws RemoteException, InterruptedException {
        ...
        if(tradingTime.equals(newTradingTime)){
            assertTrue(false, "Response caching worked even with the negative cache timeout");
        }
