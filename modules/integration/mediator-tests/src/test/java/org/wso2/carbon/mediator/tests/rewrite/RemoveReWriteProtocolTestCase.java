...
public class RemoveReWriteProtocolTestCase extends ESBIntegrationTestCase {
    private StockQuoteClient axis2Client;

    public void init() throws Exception {
        axis2Client = new StockQuoteClient();
        String filePath = "/mediators/rewrite/remove_rewrite_protocol_synapse.xml";
        loadESBConfigurationFromClasspath(filePath);
        launchBackendAxis2Service(SampleAxis2Server.SIMPLE_STOCK_QUOTE_SERVICE);

    }

    @Test(priority = 1, groups = {"wso2.esb"}, description = "Remove and rewrite protocol",
