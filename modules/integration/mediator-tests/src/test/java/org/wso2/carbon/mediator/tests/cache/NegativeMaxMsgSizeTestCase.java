...
/**
 * This class will test cache mediator which has negative value to max message size property
 * in it's configuration
 */
public class NegativeMaxMsgSizeTestCase extends ESBIntegrationTestCase {
    private StockQuoteClient axis2Client;

    public void init() throws Exception {
        axis2Client = new StockQuoteClient();
        launchBackendAxis2Service(SampleAxis2Server.SIMPLE_STOCK_QUOTE_SERVICE);
        String filePath = "/mediators/cache/negative_max_msg_size.xml";
    ...
    public void testMessageSizeProperty() throws RemoteException, InterruptedException {
        ...
        if(!tradingTime.equals(newTradingTime)){
            assertTrue(true, "Response caching having error with negative max message size value");
        }
        if(!tradingTime.equals(newTradingTime2)){
            assertTrue(false,"Response caching having error with negative max message size value");
        }
