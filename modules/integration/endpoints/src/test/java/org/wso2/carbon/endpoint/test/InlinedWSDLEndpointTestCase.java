...
public class InlinedWSDLEndpointTestCase extends ESBIntegrationTestCase {

    private static final String ENDPOINT_NAME = "wsdlEpTest";

    public InlinedWSDLEndpointTestCase() {
        super("EndpointAdmin");
    }

    @Test(groups = {"wso2.esb"})
    public void testInlineWSDLEndpoint() throws RemoteException,
    ...
    private void endpointAdditionScenario(EndpointAdminStub endpointAdminStub)
            throws RemoteException, EndpointAdminEndpointAdminException, XMLStreamException {
        int beforeCount = endpointAdminStub.getEndpointCount();

        OMElement omElement = loadClasspathResource("/wsdlTestEp.xml");
