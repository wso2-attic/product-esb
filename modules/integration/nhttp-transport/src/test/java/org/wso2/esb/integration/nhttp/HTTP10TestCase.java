...
public class HTTP10TestCase extends ESBIntegrationTestCase {
    ...
    public void testForceHttp10() throws IOException {
        ...
        assertTrue(interceptor.getVersion().compareToVersion(HttpVersion.HTTP_1_1) == 0);

        loadESBConfigurationFromClasspath("/force_http_10.xml");
        response = httpClient.doPost(getMainSequenceURL(), null, "<test/>", "application/xml");
        assertNotNull(response);
        assertTrue(interceptor.getVersion().compareToVersion(HttpVersion.HTTP_1_0) == 0);
    }
