...
public class ESBJAVA3022SendingSoapRequestAfterRestRequestTestCase extends ESBIntegrationTest {
    ...
    @Test(groups = "wso2.esb", description = "test 1 (Soap > REST > Soap) to verify that the soap builder/formatter is invoked properly" +
            "when the soap request is made after rest request.")
	public void testSendingSoap_REST_And_Soap_CallsInAChain() throws Exception {
    	String restURL = esbServer.getServiceUrl() + "/testmessagetype/soap_rest_soap";
    ...
    @Test(groups = "wso2.esb", description = "test 2 (REST > Soap) to verify that the soap builder/formatter is invoked properly" +
            "when the soap request is made after rest request.")
    public void testSendingREST_And_Soap_CallsInAChain() throws Exception {
        String restURL = esbServer.getServiceUrl() + "/testmessagetype/rest_soap";
        DefaultHttpClient httpclient = new DefaultHttpClient();
        HttpGet request = new HttpGet(restURL);
        HttpResponse response = httpclient.execute(request);
        BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
